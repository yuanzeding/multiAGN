...
def age_metal_alpha(passedFiles):
    """
    Function to extract the values of age and metallicity
    from standard XSHOOTER filenames.
    """
    ...
    for num, s in enumerate(files):
        splits = s.split('_')
        # Ages
        age = 10**(float(splits[2][4:])-9)  # age in Gyr
    ...
    # Sort the templates in a cube of age, metal, alpha for the SFH module
        ...
        for i, a in enumerate(alpha_str):
            ...
            for k, mh in enumerate(metal_str):
                files = [s for s in sp_models if ('MH'+str(mh) in s)]
                # This sorts for ages
