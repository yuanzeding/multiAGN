...
os.environ["SPS_HOME"]=HOME+"/software/fsps"


from prospect.fitting import fit_model,lnprobfn


multiAGNpath=HOME+"/multiAGN"
...
def build_obs(snr=10, ldist=10.0, **extras):
    ...
    data = np.loadtxt(os.path.join(path_out, "{}_nuclear_spec_r0.75_obj{}.txt".format(specinstru,obj)))
    ...
    zf=1+z
    mask = (specmaggies>0.001*np.median(specmaggies))  &((lam<5600*zf)| (lam>5900*zf)) & mask0#&((lam<6500*zf)| (lam>6765*zf))
...
def build_model(object_redshift=None, ldist=10.0, fixed_metallicity=None,
   ...
   # so we use that here too, since the `maggies` are appropriate for that distance.
    ...
    model_params["dust2"]["init"] = Av*0.4
    #model_params["dust2"]["prior"] = priors.TopHat(mini=1.0, maxi=8.0)
    ...
    model_params["logmass"]["prior"] = priors.TopHat(mini=8, maxi=10)
    ...
    if add_dispersion:
        ...
        model_params.update(TemplateLibrary["spectral_smoothing"])
        model_params["sigma_smooth"]["init"] = sentry["vdispe"][obj-1]
...
if __name__ == '__main__':
    ...
    run_params["add_dispersion"] = False #fix stellar+instrumental velocity dispersion 
    ...
    isoc=sps.ssp.libraries[0].decode('utf-8')
    stellib=sps.ssp.libraries[1].decode('utf-8')
    print("{},{}".format(isoc,stellib))
   ...
   # run_params['nested_target_n_effective'] = 10000
    ...
    run_params["nested_first_update"] = {'min_ncall': 2000, 'min_eff': 5.0}
    ...
    hfile = "obj{}_{}_dynesty_{}_{}.h5".format(obj, int(time.time()),isoc,stellib)
